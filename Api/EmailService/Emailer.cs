...
namespace RssCrawleraApi.Email
{
    public class Emailer : IEmailSender
    {
        ...
        public async Task SendEmailAsync(string recepientName, string recepientEmail, string subject, string body, SmtpConfig config = null, bool isHtml = true)
        {
            var from = new MailboxAddress(_config.Name, _config.EmailAddress);
            var to = new MailboxAddress(recepientName, recepientEmail);

            await SendEmailAsync(from, new MailboxAddress[] { to }, subject, body, config, isHtml);
        }

        private async Task SendEmailAsync(MailboxAddress sender, MailboxAddress[] recepients, string subject, string body, SmtpConfig config = null, bool isHtml = true)
        {
